...
CREATED:  2023-01-04 03:05:02
MODIFIED: 2023-01-04 03:05:21
"""
import cv2
import numpy as np
import math
...
def getOutputs(result_list,oriImg,data,pad):
    stride = 8
    # output heatmaps
    heatmap = np.transpose(np.squeeze([result_list[:, :, :, :19]]), (0,1,2)) 
    heatmap = cv2.resize(heatmap, (0,0), fx=stride, fy=stride, interpolation=cv2.INTER_CUBIC)
    heatmap = heatmap[:data.shape[0]-pad[2], :data.shape[1]-pad[3], :]
    heatmap = cv2.resize(heatmap, (oriImg.shape[1], oriImg.shape[0]), interpolation=cv2.INTER_CUBIC)
    # output PAFs
    paf = np.transpose(np.squeeze([result_list[:, :, :, 19:]]), (0,1,2)) 
...
def calculatePeaks(heatmap):
    ...
    for part in range(19-1):
        ...
        peaks_binary = np.logical_and.reduce((map>=map_left, map>=map_right, map>=map_up, map>=map_down, map > 0.01))
